...
Kadira.connect = function(appId, appSecret, options) {
  ...
  options.webClientEndpoint = options.webClientEndpoint || options.endpoint
  ...
  // remove trailing slash from webClientEndpoint url (if any)
  if(_.last(options.webClientEndpoint) === '/') {
    options.webClientEndpoint = options.webClientEndpoint.substr(0, options.webClientEndpoint.length - 1);
  }
  ...
  __meteor_runtime_config__.kadira = {
    appId: appId,
    endpoint: options.webClientEndpoint || options.endpoint,
